...
$sheet->setCellValue('C1', 'Номенклатура');
$sheet->setCellValue('D1', 'Наличие');
$sheet->setCellValue('E1', 'Цена опт');
$sheet->setCellValue('F1', 'Цена ррц');

$products = \R::getAll("SELECT product.article, product.name, product.quantity, product.opt_price, product.price, brand.name as brand_name FROM product, brand WHERE product.brand_id = brand.id AND product.stock_status_id = ? AND product.category_id IN (2, 9, 18, 19, 20, 21, 22, 23, 24, 26, 27, 28, 29, 30, 35) AND product.hide = ?", [1, 'show']);
...
foreach($products as $prod) {
	...
	$sheet->setCellValue('B'.$pos.'', ''.$prod["brand_name"].'');
	$sheet->setCellValue('C'.$pos.'', ''.$prod["name"].'');
	$sheet->setCellValue('D'.$pos.'', ''.$prod["quantity"].'');
	$sheet->setCellValue('E'.$pos.'', ''.$prod["opt_price"].'');
	$sheet->setCellValue('F'.$pos.'', ''.$prod["price"].'');
}
