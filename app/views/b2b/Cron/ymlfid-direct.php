...
$fd = fopen("cron/".$viewcrons["url_download"]."-".$sizename.".xml", 'w+') or die("не удалось создать файл");
    $text = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
              ...
              <shop>
				...
				<categories>
					<category id=\"1\">Шины для спецтехники</category>";
		
	$category = \R::getAll("SELECT id, name, parent_id FROM `category` WHERE id = '".$cat_id."' AND hide ='show'");
	...
	AND product.name LIKE '%".$tiposize."%' AND category.id = '".$cat_id."' AND product.hide ='show' AND product.article != '' AND product.img != '' AND product.price !='0' AND product.stock_status_id ='1'");
